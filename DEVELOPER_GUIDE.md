...
本指南面向首次接入 OrcaPlayground 开发的用户，指导完成本地环境配置与 GPU 旁路设置。
...
| `<repo-root>` | OrcaPlayground 仓库根目录 | clone 后的路径（如 `~/repo/OrcaPlayground`） |
...
OrcaPlayground 与 OrcaGym 共用单一 `orca` conda 环境（README 推荐的环境名称）。若已按 OrcaGym 指南创建过 `orca` 环境，跳过本步。
...
按 README 指引安装本项目依赖。OrcaPlayground 依赖 OrcaGym，请确保 OrcaGym 已在 `orca` 环境内以 editable 模式安装。若需 GPU 加速（Euler 体系使用 MuJoCoFlow / Flow 在 GPU 上求解），还需确保 `orca` 环境内已安装 Flow / Warp 的原生 CUDA 库。
