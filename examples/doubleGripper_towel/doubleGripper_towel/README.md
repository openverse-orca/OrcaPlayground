...
A: 这是 **OrcaGym + MuJoCo 3.7** 兼容问题，请按官方仓库安装带修复的分支：

```bash
git clone https://github.com/openverse-orca/OrcaGym.git
cd OrcaGym
git checkout feat/mujoco37-fastfall-adaptation

conda activate orca-apr24   # 或你的环境名
pip install -e .
pip install "mujoco==3.7.0"   # 覆盖 pyproject 中的旧版本约束

# 避免 import 冲突（见下一条 FAQ）
rm -rf "$CONDA_PREFIX/lib/python3.12/site-packages/orca_gym"
python -c "from orca_gym import OrcaGymLocal; print('OK')"
```

修复内容在 `orca_gym/core/orca_gym_local.py` 的 `_sanitize_local_env_xml_for_newer_mujoco()`，会自动移除 XML 中的 `vertcollide`。

若本地已有旧缓存，可删除后重试：`rm -f ~/.orcagym/tmp/*.xml`

官方仓库：[openverse-orca/OrcaGym](https://github.com/openverse-orca/OrcaGym.git)

### Q: 报错 `cannot import name 'OrcaGymLocal' from 'orca_gym' (unknown location)`
A: `pip install -e OrcaGym` 后若 `site-packages/orca_gym/` 残留空目录，会与源码冲突。删除残留目录后重试：
