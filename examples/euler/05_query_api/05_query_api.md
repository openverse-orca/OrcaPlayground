...
> 阶段四（P4）在线端到端验证第 1 课。本课验证 G1 人形机器人在 OrcaStudio/OrcaLab 真实仿真中
> 的全套状态查询 API 运行正确。
...
本目录不依赖 `envs/` 共享目录，所有 `.py` 文件均在本地，资产集中在 `examples/euler/assets/`：

```
examples/euler/05_query_api/
├── 05_query_api.md          ← 本教程
├── query_api.py             ← 脚本入口
├── query_api_env.py         ← QueryApiEnv（G1BaseEnv 子类）
├── g1_base_env.py           ← G1BaseEnv 基类（资产路径指向 examples/euler/assets/g1/）
├── scene_scanner.py         ← 场景扫描探针（OrcaGymEulerEnv，非 Local）
└── online_verifier.py       ← 在线判定器
```

**资产路径**：`g1_base_env.py` 顶部通过 `__file__` 上溯 2 层定位 `examples/euler/`，
指向 `examples/euler/assets/g1/g1_29dof_camera.xml`、`examples/euler/assets/g1/config/g1_29dof_hist.yaml`、
`examples/euler/assets/g1/models/dec_loco/model_6600.onnx`。
