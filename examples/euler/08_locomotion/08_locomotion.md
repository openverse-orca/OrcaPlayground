...
> 阶段四（P4）在线端到端验证第 8 课。本课专注验证 G1 人形机器人在 OrcaStudio/OrcaLab 真实仿真中
> 的 **ONNX 行走控制链路**（不含视频采集，视频采集见 Lesson 9）。
>
> **拆分背景**：原 Lesson 8（studio_capture）把行走控制与视频采集耦合，行走链路出问题时
...
```
OrcaPlayground/examples/euler/assets/g1/g1_29dof_camera.xml          # G1 模型（motor 执行器）
OrcaPlayground/examples/euler/assets/g1/config/g1_29dof_hist.yaml    # 配置（JOINT_KP/KD、力矩限位、默认姿态）
...
[OBSERVE] start: 请在 Studio 视口观察 G1 初始姿态：应站立在地面上
  ...
  [OBSERVE] lesson7_done: Lesson 8 行走控制验证结束，请在视口确认 G1 行走稳定性
...
- **Lesson 9**：视频录制（在行走已验证的基础上，叠加 Studio 视频/帧/时间戳采集 API 验证）
- **Lesson 10**：体操作与拖拽判定（行走中拖拽 G1、锚定后释放、观察恢复行走）
- 本课行走控制链路是 Lesson 9/10 的前置依赖，必须先跑通
