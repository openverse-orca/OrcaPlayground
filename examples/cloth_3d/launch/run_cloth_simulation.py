#!/usr/bin/env python3
"""cloth_3d 主编排：MuJoCo 宏步 + OrcaLink 锚点发布。"""
...
def _start_orcalink_server(ol_cfg: dict) -> subprocess.Popen | None:
    ...
    repo = Path(__file__).resolve().parents[4]  # .../OrcaApr24
    for rel in (
...
def main() -> int:
    parser = argparse.ArgumentParser(description="cloth_3d MuJoCo + OrcaLink anchor publish")
    parser.add_argument(
        "--config",
        type=Path,
