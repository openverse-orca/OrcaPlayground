...
├── .gitignore                      # 忽略训练产物
├── 00_setup.md                     # 阶段四（Lesson 5-10）环境搭建
├── 01_hello_euler/                 # 第 1 课（P3 离线联调）
├── 02_online_render/               # 第 2 课（P3A 在线渲染）
├── 03_env_register/                # 第 3 课（P3B 环境注册与 ID 启动）
├── 04_rl_ppo/                      # 第 4 课（P3B SB3 PPO 强化学习）
├── 05_query_api/                   # 第 5 课（P4 状态查询 API）
├── 06_force_apply/                 # 第 6 课（P4 外力应用与约束）
├── 07_jacobian/                    # 第 7 课（P4 雅可比 IK 与 mocap）
├── 08_locomotion/                  # 第 8 课（P4 G1 行走控制链路）
├── 09_video_capture/               # 第 9 课（P4 Studio 视频录制与截帧）
├── 10_body_manipulation/           # 第 10 课（P4 体操作与 equality）
└── 11_scene_config/                # 第 11 课（P4 求解器与场景配置深度）
...
│       └── mimic/                     # mimic 数据
└── scenes/
    └── simple_pendulum.xml         # 第 1-4 课场景
```

> **自包含原则**：每个 example 目录包含全部所需 `.py` 文件，import 使用同目录引用，
> 资产路径通过 `__file__` 上溯 2 层定位 `examples/euler/assets/`。零 `envs/` 共享目录依赖，
...
| 3 | P3B | 环境注册与 ID 启动 | `gym.register`、`gym.make`、entry_point、幂等性、env ID 管理 | ✅ | 是 | [03_env_register.md](03_env_register/03_env_register.md) |
| 4 | P3B | SB3 PPO 强化学习 | Gymnasium API 契约、SB3 PPO、奖励函数设计、Box 观测、episode 截断 | ✅ | 否（离线训练） | [04_rl_ppo.md](04_rl_ppo/04_rl_ppo.md) |

### 阶段四：在线端到端验证（Lesson 5-10，需 OrcaStudio/OrcaLab + G1 关卡）
...
| 课 | 阶段 | 主题 | 新概念 | 状态 | 教程 |
|----|------|------|--------|------|------|
| 5 | P4 | 状态查询 API | `query_joint_*`、`query_site_*`、`query_sensor_data`、`query_contact_*` | ✅ | [05_query_api.md](05_query_api/05_query_api.md) |
| 6 | P4 | 外力应用与约束 | `apply_body_force`、`clear_*_force`、`update_equality_constraints`、`set_*` | ✅ | [06_force_apply.md](06_force_apply/06_force_apply.md) |
| 7 | P4 | 雅可比 IK 与 mocap | `mj_jacBody`、`mj_jacSite`、`set_mocap_pos_and_quat`、`anchor_actor` | ✅ | [07_jacobian.md](07_jacobian/07_jacobian.md) |
| 8 | P4 | G1 行走控制链路 | ONNX 推理、PD 控制器、行走稳定性、力矩触限 | ✅ | [08_locomotion.md](08_locomotion/08_locomotion.md) |
| 9 | P4 | Studio 视频录制与截帧 | `get_current_frame`、`get_next_frame`、`get_frame_png`、视频录制 | ✅ | [09_video_capture.md](09_video_capture/09_video_capture.md) |
| 10 | P4 | 体操作与 equality | `equality_find_slot_by_body`、`equality_constraint`、`equality_update`、mocap 拖拽 + 行走 | ✅ | [10_body_manipulation.md](10_body_manipulation/10_body_manipulation.md) |

### 补充课程

| 课 | 阶段 | 主题 | 新概念 | 状态 | 教程 |
|----|------|------|--------|------|------|
| 11 | P4 | 求解器与场景配置深度 | `SimConfig`（timestep/integrator/iterations/gravity）、能量保持、接触求解 | ✅ | [11_scene_config.md](11_scene_config/11_scene_config.md) |
...
Euler 仿真的标准 Gymnasium 流程。本课为在线模式（连接 OrcaStudio），验证 env ID
注册、entry_point 动态导入、幂等性等生产环境必备能力。

- **运行**：`python examples/euler/03_env_register/run_env_register.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 pendulum 场景）
- **预计时长**：< 10 秒

📖 详细教程见 [03_env_register/03_env_register.md](03_env_register/03_env_register.md)
...
- **运行**：`python examples/euler/04_rl_ppo/train_ppo.py --total-timesteps 100000`
- **需要 OrcaStudio/OrcaLab**：否（训练离线，评估可选 online）
- **预计时长**：100k 步约 2-3 分钟（GPU）

📖 详细教程见 [04_rl_ppo/04_rl_ppo.md](04_rl_ppo/04_rl_ppo.md)
...
- **运行**：`python examples/euler/05_query_api/query_api.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 G1 关卡）
- **预计时长**：< 1 分钟

📖 详细教程见 [05_query_api/05_query_api.md](05_query_api/05_query_api.md)

---

### 第 6 课：外力应用与约束 — 施力、摩擦、状态写入

验证外力应用 API（`apply_body_force` / `clear_*_force`）、摩擦设置（`set_geom_friction`）、
状态写入（`set_joint_qpos` / `set_joint_qvel`）。通过对照实验验证外力效果。

- **运行**：`python examples/euler/06_force_apply/force_apply.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 G1 关卡）
- **预计时长**：< 1 分钟

📖 详细教程见 [06_force_apply/06_force_apply.md](06_force_apply/06_force_apply.md)

---

### 第 7 课：雅可比 IK 与 mocap — 逆运动学控制

验证雅可比矩阵计算（`mj_jacBody` / `mj_jacSite`）、Site 查询、Mocap 控制
（`set_mocap_pos_and_quat`）、锚点操作（`anchor_actor`）。通过 IK 求解驱动 G1 手部
追踪目标轨迹。

- **运行**：`python examples/euler/07_jacobian/jacobian_ik.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 G1 关卡）
- **预计时长**：< 1 分钟

📖 详细教程见 [07_jacobian/07_jacobian.md](07_jacobian/07_jacobian.md)
...
聚焦行走控制本身，不耦合视频采集（视频采集见 Lesson 9）。

- **运行**：`python examples/euler/08_locomotion/locomotion.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 G1 关卡）
- **预计时长**：< 1 分钟

📖 详细教程见 [08_locomotion/08_locomotion.md](08_locomotion/08_locomotion.md)
...
`get_frame_png`），复用 Lesson 8 的行走控制链路驱动 G1 行走并录制。

- **运行**：`python examples/euler/09_video_capture/video_capture.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 G1 关卡）
- **预计时长**：约 10 秒（录制 5 秒视频 + 截图）

📖 详细教程见 [09_video_capture/09_video_capture.md](09_video_capture/09_video_capture.md)
...
- **运行**：`python examples/euler/10_body_manipulation/body_manipulation.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 G1 关卡）
- **预计时长**：交互式，取决于用户输入

📖 详细教程见 [10_body_manipulation/10_body_manipulation.md](10_body_manipulation/10_body_manipulation.md)

---

### 第 11 课：求解器与场景配置深度 — SimConfig 四属性对比实验

通过四组对比实验（timestep、integrator、gravity、iterations）验证 `SimConfig` 对
MuJoCo 求解器的影响，观察能量保持、积分器特性、接触求解精度等。使用单摆系统
进行 20 秒长时仿真，放大不同配置下的能量漂移差异。

- **运行**：`python examples/euler/11_scene_config/run_scene_config.py`
- **需要 OrcaStudio/OrcaLab**：是（加载 pendulum 场景）
- **预计时长**：约 1 分钟

📖 详细教程见 [11_scene_config/11_scene_config.md](11_scene_config/11_scene_config.md)
...
| OrcaStudio/OrcaLab | **第 2-3 课、第 5-10 课需要**，需启动 gRPC 服务（默认 `localhost:50051`） |
| stable-baselines3 | **仅第 4 课需要**，`requirements.txt` 已包含 |
| **Euler_asset 资产包** | **第 9 课需要**，需在 OrcaStudio/OrcaLab 中订阅（见下） |

#### 订阅 Euler_asset 资产包（第 9 课必需）

第 9 课（视频捕获）通过 `OrcaGymScene.add_actor` + `publish_scene` 在线 spawn G1 与
...
5. 运行 `python examples/euler/09_video_capture/video_capture.py`
...
### Q2：第 4 课训练时出现 `UserWarning: You are trying to run PPO on the GPU`
...
### Q6：第 9 课运行报错 "所有 spawnable 候选路径均失败"

**原因**：第 9 课通过 `OrcaGymScene.add_actor` 在线 spawn G1 + 50 个障碍物，所需的
