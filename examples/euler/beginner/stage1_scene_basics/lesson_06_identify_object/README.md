...
## 小挑战
先用 `"table"` 检索：命中几个？再用 `"ball"` 检索对比。为什么换个
关键词，命中的 body 完全不同——`table` 为什么一个 `ball_1` 都轮不到？
（提示：检索按名称**子串匹配**，和物体是什么没有关系。）

## 阶段 1 毕业挑战 🎓
搭一个**你自己的角落**：改 `build_recipe()`，用地面 + 桌子 + 至少 2 个
你选的物体组成一个小场景（可从第 02 课的可选物体里挑，或参考
`_common/assets.py` 换成资产库里的其它资产）；再把 `SEARCH_KEYWORD`
换成你的物体名，确认检索全部命中。
