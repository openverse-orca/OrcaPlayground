"""2.1.2 入口脚本：加载 USD 物体资产（三张桌子）。

时序 spawn desk_1 → desk → desk_2，以原点为中心并排布局。
...
用法:
    ...
    # 自定义间距与间隔
    python examples/scene_building/01_assets/02_load_usd_object/run_load_usd_object.py --spacing 3.0 --interval 5
...
验证点:
    1. t=0s 视口出现 desk_1（左侧）
    2. t=3s 视口出现 desk_1 + desk（中间）
    3. t=6s 视口出现 desk_1 + desk + desk_2（右侧）
    4. 三桌子并排，间距 2.0m
"""
...
from load_usd_object import SPAWN_INTERVAL, SPACING, load_usd_object  # noqa: E402
...
def main() -> None:
    ...
    _log(
        f"时序 spawn 三张桌子 @ {args.addr}（间距 {args.spacing:.2f}m，间隔 {args.interval:.1f}s）"
    )

    # 1. Studio rundata：开始
    sceneinfo(args.addr, "beginscene")

    # 2. 时序 spawn 三张桌子（append_scene 增量 spawn）
