#!/usr/bin/env python3
"""
生成 dual_gripper_rect_yixuan_h03 MuJoCo 场景：台面 12m×6m 不变；夹爪初始对齐 20×10 布短轴顶点。
"""
...
from modules.dual_gripper_rect_sheet_geom import CLOTH_SPACING_M, sheet_axis_extents_m  # noqa: E402
...
CLOTH_NX = 20
CLOTH_NY = 10
_X_NEG, _, _Z_MIN, _Z_MAX = sheet_axis_extents_m(CLOTH_NX, CLOTH_NY, CLOTH_SPACING_M)
GRIP_L0 = (_X_NEG, 1.50, _Z_MIN)
GRIP_R0 = (_X_NEG, 1.50, _Z_MAX)
...
def main() -> int:
    ...
    xml = f"""<mujoco model="dual_gripper_rect_yixuan_h03">
  <!-- gen_dual_gripper_rect_yixuan_h03_scene.py：20×10 布，台面 12×6m 不变 -->
...
"""
    out_dir = ROOT / "assets" / "dual_gripper_rect_yixuan_h03"
    mesh_dir = out_dir / "meshes"
    mesh_dir.mkdir(parents=True, exist_ok=True)
    src_mesh = ROOT / "assets" / "dual_gripper_rect_yixuan_h1x" / "meshes"
    for name in ("gripper_palm.obj", "gripper_finger.obj"):
        src = src_mesh / name
        dst = mesh_dir / name
        if src.is_file():
