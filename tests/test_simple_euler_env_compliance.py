...
运行方式:
    <conda-base>/envs/orca/bin/python -m unittest tests.test_simple_euler_env_compliance
"""
...
# simple_env.py 位于 OrcaPlayground/examples/euler/01_hello_euler/（新目录结构）
# __file__ = OrcaPlayground/tests/test_simple_euler_env_compliance.py
# parents[0] = OrcaPlayground/tests
# parents[1] = OrcaPlayground
_SIMPLE_ENV_PATH = (
    pathlib.Path(__file__).resolve().parents[1]
    / "examples" / "euler" / "01_hello_euler" / "simple_env.py"
)
...
class TestSimpleEnvResetModelFunctional(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # 新目录结构：examples/euler/01_hello_euler/simple_env.py
        # 通过 importlib 直接从文件路径加载，避免包路径问题
        _simple_env_path = (
            pathlib.Path(__file__).resolve().parents[1]
            / "examples" / "euler" / "01_hello_euler" / "simple_env.py"
        )
        spec = importlib.util.spec_from_file_location("simple_env", _simple_env_path)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        cls.SimpleEulerEnv = module.SimpleEulerEnv
        cls.env = module.SimpleEulerEnv()
