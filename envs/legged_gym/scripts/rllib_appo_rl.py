...
warnings.warn(
    "envs/legged_gym/scripts/rllib_appo_rl.py 已迁移至 examples/legged_gym/scripts/rllib_appo_rl.py，"
    "此文件将在未来版本中移除。",
    DeprecationWarning,
    stacklevel=2,
)
...
def get_config(
    ...
    print(
        f"num_learners: {num_learners}, rollout_fragment_length: {rollout_fragment_length}, "
        f"minibatch_size: {minibatch_size}, train_batch_size_per_learner: {train_batch_size_per_learner}"
    )
    ...
    config = (
        ...
        .env_runners(
            ...
            if async_env_runner
            else gym.envs.registration.VectorizeMode.SYNC,
    ...
    config.model["fcnet_activation"] = "silu"
    config.model["free_log_std"] = free_log_std
    config.model["vf_share_layers"] = vf_share_layers
    ...
    config = (
        ...
        .training(
            ...
            circular_buffer_iterations_per_batch=agent_config.get(
