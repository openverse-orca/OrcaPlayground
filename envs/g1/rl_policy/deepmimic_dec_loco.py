...
class MotionTrackingDecLocoPolicy(BasePolicy):
    ...
    def _get_obs_phase_time(self):
        cur_time = time.time() * self.stand_command[0, 0]
    ...
    def get_policy_action(self, robot_state_data):
        if self.policy_locomotion_mimic_flag:
            ...
            if not self.interpolation_done and self.interpolation_progress < self.interpolation_threshold_loco2mimic:
                timestep = time.time() - self.interpolation_start_time
                ...
                if self.interpolation_progress >= self.interpolation_threshold_loco2mimic:
                    ...
                    orca_logger.info(f"\rSwitched to Mimic policy: {self.policy_mimic_names[self.policy_mimic_idx]}")
                    self.history_handler.reset([0])
                    # 重置 last_action，mimic 策略需要从 0 开始
                    self.last_action = np.zeros((1, self.num_dofs))
                    self._mimic_debug_count = 0  # 重置调试计数器
                    # import ipdb; ipdb.set_trace()
                else:
            ...
            if self.interpolation_done:
                ...
                if self.policy_locomotion_mimic_flag:
                    ...
                    # 与 ASAP 一致，使用 -100, 100 的 clip 范围
                    policy_action = np.clip(raw_action, -100, 100)
                    ...
                    # 调试：打印第一帧的详细信息
                    if not hasattr(self, '_mimic_debug_count'):
                        self._mimic_debug_count = 0
                    if self._mimic_debug_count < 5:
                        print(f"\n=== Mimic Frame {self._mimic_debug_count} ===")
                        print(f"phase={self.phase:.4f}")
                        print(f"obs: shape={obs.shape}, min={obs.min():.4f}, max={obs.max():.4f}")
                        print(f"raw_action: shape={raw_action.shape}, min={raw_action.min():.4f}, max={raw_action.max():.4f}")
                        print(f"scaled_action: min={scaled_policy_action.min():.4f}, max={scaled_policy_action.max():.4f}")
                        # 打印 obs 各组件
                        idx = 0
                        n_action = policy_action.shape[1]  # 23
                        print(f"  last_action[0:{n_action}]: {obs[0, idx:idx+n_action].min():.4f} ~ {obs[0, idx:idx+n_action].max():.4f}")
                        idx += n_action
                        print(f"  base_ang_vel[{idx}:{idx+3}]: {obs[0, idx:idx+3].min():.4f} ~ {obs[0, idx:idx+3].max():.4f}")
                        idx += 3
                        print(f"  dof_pos[{idx}:{idx+n_action}]: {obs[0, idx:idx+n_action].min():.4f} ~ {obs[0, idx:idx+n_action].max():.4f}")
                        idx += n_action
                        print(f"  dof_vel[{idx}:{idx+n_action}]: {obs[0, idx:idx+n_action].min():.4f} ~ {obs[0, idx:idx+n_action].max():.4f}")
                        idx += n_action
                        hist_len = obs.shape[1] - idx - 3 - 1  # projected_gravity + phase
                        print(f"  history[{idx}:{idx+hist_len}]: {obs[0, idx:idx+hist_len].min():.4f} ~ {obs[0, idx:idx+hist_len].max():.4f}")
                        idx += hist_len
                        print(f"  projected_gravity[{idx}:{idx+3}]: {obs[0, idx:idx+3]}")
                        idx += 3
                        print(f"  phase[{idx}]: {obs[0, idx]:.4f}")
                        self._mimic_debug_count += 1
                else:
        ...
        if not self.policy_locomotion_mimic_flag or not self.interpolation_done:
            ...
            if self.interpolation_done or self.interpolation_emergency:
                ...
                timestep = time.time() - self.frame_start_time
    ...
    def _handle_keyboard_button_impl(self, keycode):
        ...
        if keycode == "[":
            ...
            if self.policy_locomotion_mimic_flag == 1:
                # 切换到 mimic 策略，确保启用策略控制
                self.use_policy_action = True
                self.get_ready_state = False
                # reset interpolation variables
                self.interpolation_done = False
                orca_logger.info("Switching to Mimic policy mode")
            else: 
                self.interpolation_emergency = True
                if self.robot_state_data is not None:
                    self.end_upper_dof_pos = self.robot_state_data[:, (7+self.num_lower_dofs):(7+self.num_dofs)].copy()
                    self.ref_upper_dof_pos[0, :] = self.end_upper_dof_pos[0, :].copy()
        ...
        try:
            if cur_key == "select":
                ...
                if self.policy_locomotion_mimic_flag == 1:
                    ...
                    self.get_ready_state = False
                    self.interpolation_done = False
                    orca_logger.info(colored("Switching to Mimic policy mode", "blue"))
                else: 
                    self.interpolation_emergency = True
                    if self.robot_state_data is not None:
                        self.end_upper_dof_pos = self.robot_state_data[:, (7+self.num_lower_dofs):(7+self.num_dofs)].copy()
                        self.ref_upper_dof_pos[0, :] = self.end_upper_dof_pos[0, :].copy()
                    orca_logger.info(colored("Switching back to Locomotion policy", "blue"))
            elif cur_key == "R1":
                self.frame_start_time = time.time()
                self.next_mimic_policy()
                orca_logger.info(colored(f"Current Mimic: {self.policy_mimic_names[self.policy_mimic_idx]}, length: {self.motion_length_s[self.policy_mimic_idx]}", "blue"))
                orca_logger.info(colored(f"Current checkpoint path: {self.mimic_model_paths}", "blue"))
            elif cur_key == "L1":
                self.frame_start_time = time.time()
                self.last_mimic_policy()
                orca_logger.info(colored(f"Current Mimic: {self.policy_mimic_names[self.policy_mimic_idx]}, length: {self.motion_length_s[self.policy_mimic_idx]}", "blue"))
